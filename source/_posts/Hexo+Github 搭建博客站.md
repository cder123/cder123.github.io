...
-   Windows 10
-   Github网站：[GitHub](https://github.com/)
-   Node.js：[Node.js](https://nodejs.org/en/download/)
-   gitBash：[Git for Windows](https://gitforwindows.org/)
-   hexo：[Hexo-zh](https://hexo.io/zh-cn/)
...
-   [gitBash 安装配置 ](https://www.runoob.com/git/git-install-setup.html): 安装完后，配置git的ssh，并关联到 github
...
1.  在Github网站上建立1个名为：`cder123.github.io` 的`仓库`，在建立仓库时，勾选初始化按钮。

>   其中：
>
>   -   `cder123`为 `github的用户名`。
...
// 2. 利用Hexo创建1个名为myBlogs的目录【作为：根目录】，专门存放博客站点的所有文件
	hexo init myBlogs
...
// 4. 安装hexo的依赖插件，生成了【node_modules目录】
	npm install
...
```bash
.
├── _config.yml				// 博客站的配置文件
├── package.json			// 依赖环境
├── node_modules			// hexo的依赖包目录【npm下载的 主题、插件的 默认存放位置】
├── scaffolds				// 模板
├── source					// 博客源文件【.md文件必须全放 /source/_post 目录 下】
|   ├── _drafts				// 草稿
|   └── _posts				// 博客的MarkDown源文件存放的目录
└── themes					// 存放主题【npm下载的主题默认在 node_modules目录】
```
...
-   官方主题：
-   butterfly主题：[Butterfly 安装文档](https://butterfly.js.org/posts/21cfbf15/#安裝)
...
# Site
title: Cyw的笔记栈	# 需要更改的位置
subtitle: '笔记-让生活更便捷'	# 需要更改的位置
description: ''
keywords:
author: CYW	# 需要更改的位置
language: zh-CN	# 需要更改的位置
timezone: 'Asia/Shanghai'	# 需要更改的位置

# URL
## Set your site url here. For example, if you use GitHub Page, set url as 'https://username.github.io/project'
url: http://cder123.github.io	# 需要更改的位置
...
theme: butterfly	# 需要更改的位置

# Deployment
## Docs: https://hexo.io/docs/one-command-deployment
deploy:	# 需要更改的位置
  type: git
  repository: git@github.com:cder123/cder123.github.io.git
  branch: main	# 备注：github默认的分支为main
...
-   标签页：
    ...
    ```bash
    // 找到source/tags/index.md这个文件
    
    
    ---
    title: 标签
    date: 2018-01-05 00:00:00	// 日期可以改成别的
    type: "tags"
    ---
    ```
...
// 找到 source/categories/index.md这个文件,输入：
...
将butterfly主题目录【默认在node_modules目录下】下的`_conf.yml` 文件 **改名** 为`_config.butterfly.yml`，并放到`站点的根目录`下。
...
# Favicon（網站圖標）
favicon: https://z3.ax1x.com/2021/06/01/2KeKMQ.png

# Avatar (頭像)
avatar:
  img: https://z3.ax1x.com/2021/06/01/2KZgDs.png
  effect: true
...
default_top_img: https://z3.ax1x.com/2021/06/01/2uBDbR.png
...
cover:
  ...
  default_cover:
     - https://z3.ax1x.com/2021/06/01/2KV1yj.png
...
aside:
  ...
  card_author:
    ...
    button:
      ...
      icon: fab fa-github
      text: 点我吧
      link: https://cder123.gitee.io
  card_announcement:
    enable: true
    content: 本站点基于Hexo和GitPages技术搭建，站点内容为本人搜集或原创的笔记。
  card_recent_post:
    enable: true
...
因此，本地可以在执行完hexo的相关命令后，直接添加，提交，推送到备份分支。

即：

-   `main分支：` 部署站点
-   `master分支：`备份站点文件，master分支是自己创建的。



执行`hexo clean`，`hexo g`，`hexo d`后，执行git操作，备份文件。
